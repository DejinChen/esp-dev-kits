...
void matter_ctrl_change_state(lv_obj_t *obj)
{
    device_list_lock my_device_lock;
    if (NULL == obj) {
        return;
    ...
    if (dev_ptr->device_type == CONTROL_LIGHT_DEVICE || dev_ptr->device_type == CONTROL_PLUG_DEVICE) {
        /* for light and plug matter device, on/off server is supported */
        dev_ptr->OnOff = !dev_ptr->OnOff;
        ui_set_onoff_state(dev_ptr->lv_obj, dev_ptr->device_type, dev_ptr->OnOff);
        chip::DeviceLayer::PlatformMgr().ScheduleWork(send_command_cb, (intptr_t)dev_ptr);
    }
}
